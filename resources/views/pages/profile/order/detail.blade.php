...
@section('content')
    <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pt-36 pb-24">
        <div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
            <h1 class="text-xl font-bold text-gray-800 mb-2">Detail Pesanan</h1>
            <div class="flex items-center gap-2">
                <span class="text-gray-600">Kode Booking:</span>
                <span class="font-semibold text-lime-600">{{ $booking->booking_code }}</span>
            </div>
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4 pt-4 border-t">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">{{ $booking->user->email }}</p>
                            </div>
                            <div>
                                <p class="text-gray-500 text-sm">No. Telpon</p>
                                <p class="font-semibold text-gray-800">{{ $booking->user->phone ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-gray-500 text-sm">Alamat</p>
                                <p class="font-semibold text-gray-800">{{ $booking->user->address ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4 pt-4 border-t">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">
                                    {{ $booking->vehicle->vehicleModel->category->name }}</p>
                            </div>
                            <div>
                        ...
                        <div class="pt-4 border-t">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="font-semibold text-gray-800">{{ $booking->pickup_location }}</p>
                ...
                <form action="{{ route('bookings.pay', $booking) }}" method="POST"
                    ...
                    @if (in_array($status, ['pending', 'failed']))
                            ...
                            <div x-data="{ paymentType: 'dp' }" class="space-y-2">
                                <label
                                    ...
                                    <input type="radio" name="payment_type" value="dp"
                                        class="w-4 h-4 text-lime-600" x-model="paymentType">
                                    <span class="ml-3 font-semibold text-gray-800">DP 30% (Rp
                                        {{ number_format($booking->deposit_amount, 0, ',', '.') }})</span>
                                </label>
                                <label
                                    ...
                                    <input type="radio" name="payment_type" value="full"
                                        class="w-4 h-4 text-lime-600" x-model="paymentType">
                                    <span class="ml-3 font-semibold text-gray-800">Full Payment (Rp
                                        {{ number_format($booking->total_price, 0, ',', '.') }})</span>
                                </label>
                            </div>
                        </div>
                        <button type="submit"
                            ...
                            {{ $status === 'failed' ? 'Bayar Ulang' : 'Bayar Sekarang' }}
                        </button>
                        <p class="text-xs text-gray-500 text-center my-4">
                            Dengan melanjutkan, Anda menyetujui syarat dan ketentuan yang berlaku
                        </p>
                    @endif
                    @if ($status === 'paid')
                        <div
                            class="w-full bg-lime-50 border border-lime-200 text-lime-700 font-semibold py-4 rounded-xl text-center">
                            Pembayaran Berhasil
                        </div>
                    @endif
                    @if ($status === 'refunded')
                        <div
                            class="w-full bg-yellow-50 border border-yellow-200 text-yellow-700 font-semibold py-4 rounded-xl text-center">
                            Dana Telah Direfund
                        </div>
                    @endif
                </form>
                @if ($booking->rental_status !== 'cancelled' && !in_array($status, ['paid', 'refunded']))
                    <form action="{{ route('orders.cancel', $booking) }}" method="POST" class="mt-4">
                        @csrf
                        <button type="submit" onclick="return confirm('Yakin ingin membatalkan pesanan ini?')"
                            class="w-full border border-red-300 text-red-600 hover:bg-red-50 font-semibold py-3 rounded-xl transition-colors">
                            Batalkan Pesanan
                        </button>
                    </form>
                @endif
